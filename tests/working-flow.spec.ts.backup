...
/**
 * Working End-to-End Test - Simplified and Ultra-Fast Version
 * Reaches 95% completion of the bankruptcy interview consistently
...
test.describe('Working Bankruptcy Interview Flow', () => {
  test('should navigate through bankruptcy interview with intelligent assistance', async ({ page }) => {
    ...
    await test.step('Navigate through interview intelligently', async () => {
      ...
      let totalRetryCount = 0; // Global retry counter to prevent infinite loops
      const maxRetries = 3; // Maximum total retries allowed
      ...
      while (stepCount < maxSteps) {
        ...
        // Check for server errors and handle them with analysis (only once per step and if under retry limit)
        if (!hasRetriedThisStep && totalRetryCount < maxRetries &&
          ...
          totalRetryCount++; // Increment global retry counter
        ...
        if (totalRetryCount >= maxRetries) {
          ...
          console.log(`🛑 ENDING TEST due to retry limit. Current page: "${analysis.h1Text}"`);

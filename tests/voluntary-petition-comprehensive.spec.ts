...
// Test 22: State-aware exemption validation
test('Variant 17: Nebraska address shows Nebraska exemptions', async ({ page }) => {
  console.log('🧪 Testing Nebraska state-aware exemptions');
  ...
  const testData = {
    ...
    amended_filing: false,
    filing_status: 'Filing individually',
    'debtor[0].name.first': 'ExemptionTest',
    'debtor[0].name.last': 'Nebraska',
    'debtor[0].address.address': '123 Exemption St',
    'debtor[0].address.city': 'Omaha',
    'debtor[0].address.state': 'Nebraska',
    'debtor[0].address.county': 'Douglas County',
    'debtor[0].address.zip': '68102',
    'debtor[0].has_other_mailing_address': false,
    'debtor[0].tax_id.tax_id_type': '1',
    'debtor[0].tax_id.tax_id': '111-11-1111',
    'debtor[0].alias.there_are_any': false,
    'debtor[0].district_info.is_current_district': true,
    'prop.interests.there_are_any': true,
    'ab_vehicles.there_are_any': true
  };
  
  // Navigate to property section where exemptions appear
  await navigateToQuestion(page, 'property', testData);
  
  // Continue to vehicles to get exemption dropdowns
  let currentStep = 0;
  const maxSteps = 10;
  
  while (currentStep < maxSteps) {
    const heading = await page.locator('h1#daMainQuestion').textContent();
    console.log(`Exemption navigation step ${currentStep}: ${heading}`);
    
    // Handle property interest question
    if (heading && heading.includes('Do you own or have any legal or equitable interest')) {
      console.log('🏠 Clicking Yes for property interests');
    ...
    // Handle vehicle question
    if (heading && heading.includes('Do you own, lease, or have legal')) {
      console.log('🚗 Clicking Yes for vehicles');
      await page.click('input[value="True"]');
      await page.click('button[type="submit"], input[type="submit"]');
      await page.waitForLoadState('networkidle');
      currentStep++;
    ...
    // Check for exemption dropdowns
    const exemptionSelects = page.locator('select[name*="exemption_laws"]');
    const exemptionCount = await exemptionSelects.count();
    
    if (exemptionCount > 0) {
      console.log(`🎯 Found ${exemptionCount} exemption dropdown(s)!`);
      
      // Get options from first dropdown
      const options = await exemptionSelects.first().locator('option').allTextContents();
      console.log('📋 First 5 exemption options:', options.slice(0, 5));
      
      // Verify Nebraska-specific exemptions are present
      const hasNebraska = options.some((option: string) => 
        option.includes('Neb. Rev. Stat.') || option.includes('Motor vehicle (Neb.')
      );
      
      console.log(`✅ Nebraska exemptions found: ${hasNebraska}`);
      expect(hasNebraska).toBeTruthy();
      
      // Take screenshot for documentation
      await page.screenshot({ 
        path: 'test-results/nebraska-exemptions-validation.png', 
        fullPage: true 
      });
      
      console.log('✅ Nebraska exemption test completed successfully');
      return;
    }
  ...
  throw new Error('Could not find exemption dropdowns for Nebraska test');
});

// Test 23: South Dakota exemption validation  
test('Variant 18: South Dakota address shows South Dakota exemptions', async ({ page }) => {
  console.log('🧪 Testing South Dakota state-aware exemptions');
  
  const testData = {
    introduction_screen: true,
    current_district: 'District of South Dakota',
    amended_filing: false,
    filing_status: 'Filing individually',
    'debtor[0].name.first': 'ExemptionTest',
    'debtor[0].name.last': 'SouthDakota',
    'debtor[0].address.address': '456 Exemption Ave',
    'debtor[0].address.city': 'Sioux Falls',
    'debtor[0].address.state': 'South Dakota',
    'debtor[0].address.county': 'Minnehaha County',
    'debtor[0].address.zip': '57104',
    'debtor[0].has_other_mailing_address': false,
    'debtor[0].tax_id.tax_id_type': '1',
    'debtor[0].tax_id.tax_id': '222-22-2222',
    'debtor[0].alias.there_are_any': false,
    'debtor[0].district_info.is_current_district': true,
    'prop.interests.there_are_any': true,
    'ab_vehicles.there_are_any': true
  ...
  while (currentStep < maxSteps) {
    ...
    if (exemptionCount > 0) {
      ...
      // Verify South Dakota-specific exemptions are present
      const hasSouthDakota = options.some((option: string) => 
        option.includes('SDCL') || option.includes('South Dakota')
      );
      
      // Verify Nebraska exemptions are NOT present
      const hasNebraska = options.some((option: string) => 
        option.includes('Motor vehicle (Neb.')
      );
      
      console.log(`✅ South Dakota exemptions found: ${hasSouthDakota}`);
      console.log(`✅ Nebraska exemptions absent: ${!hasNebraska}`);
      
      expect(hasSouthDakota).toBeTruthy();
      expect(hasNebraska).toBeFalsy();
      
      // Take screenshot for documentation
      await page.screenshot({ 
        path: 'test-results/south-dakota-exemptions-validation.png', 
        fullPage: true 
      });
      
      console.log('✅ South Dakota exemption test completed successfully');
      return;
    }
  ...
  throw new Error('Could not find exemption dropdowns for South Dakota test');
});
